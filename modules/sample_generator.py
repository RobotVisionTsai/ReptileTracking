...
#                     filename='/home/rvl224/SampleGenerator.txt')
...
class SampleGenerator():
    ...
    def __call__(self, bb, n):
        ...
        if self.aspect_f is not None:
            # ratio \in [-1, 1] aspect_f = 1.1
            # width and height would be between [0.9, 1.1]
            ratio = np.random.rand(n,1)*2-1
            samples[:,2:] *= self.aspect_f ** np.concatenate([ratio, -ratio],axis=1)

        # sample generation
        # translation depends on the size of the target by factor of trans_f
        # scale depends on scale_f, if it is 1, then [0.9,1.1]
        ...
        # adjust bbox range
        # centor location of samples would be [10, img_size-10]
