...
def crop_image(img, bbox, img_size=107, padding=16, valid=False):
    ...
    # scaled = cv2.resize(cropped, (img_size, img_size), interpolation=cv2.INTER_AREA)
    # scaled = np.array(Image.fromarray(cropped).resize(size=(img_size, img_size)))

    # cv2.imshow('imresize',scaled)
    return scaled
