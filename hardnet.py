...
class HarDNet(nn.Module):
    def __init__(self, depth_wise=False, arch=85, pretrained=True, weight_path='', num_class = 1000):
        super().__init__()
        first_ch  = [32, 64]
        second_kernel = 3
        max_pool = True
        ...
        if pretrained:
          if hasattr(torch, 'hub'):
            ...
            self.load_state_dict(torch.hub.load_state_dict_from_url(checkpoint, progress=False),strict=False)
          else:
